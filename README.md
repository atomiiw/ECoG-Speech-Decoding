...
|---|---|---|
| **PCA + SVM** (linear baseline) | Built and trained the entire pipeline from scratch | Best per patient × position: **52%**. Range **7 to 52%** across 8 patients × 3 phoneme positions. Takeaway: middle phonemes classify better than first or last, and per patient variance is huge. |
| **Seq2Seq RNN** (Cogan et al., 2023) | Used the existing architecture, ran a 40 run hyperparameter sweep (2 learning rates × 2 hidden sizes × 10 folds) | Best test **60%**, test range **21 to 60%**. Train **100%** (memorization), val 17 to 46%. Takeaway: the model memorizes the training set but doesn't generalize; data size, not architecture, is the bottleneck. |
| **MIRepNet** (Wu et al., 2025) | Froze conv and transformer layers, fine tuned a new 9 class classification head | Same patient unseen: **55%**. Cross patient: **~12%** (chance). Takeaway: fine tuning works within a patient but fails when transferred to new patients. |
| **Spatiotemporal CNN** (Flinker et al., 2024) | (1) Trained the encoder on Flinker's HB02 patient by running their full pipeline as published (a2a + e2a, 60 epochs each). (2) Removed Flinker's audio synthesis decoder. (3) Replaced it with a phoneme classification head (PCA + LDA / SVM). (4) Froze the encoder, applied the new head to Cogan S14 phoneme data. | Encoder features: **27.59%** (best, Encoder + PCA + LDA). Raw ECoG with same head: **12 to 14%**. Encoder is **~2x raw**. Takeaway: the encoder helps even on a different patient and a different task, so the CNN learned general ECoG structure. |
